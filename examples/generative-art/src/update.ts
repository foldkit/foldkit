  ...
  M.value(message).pipe(
    ...
    M.tagsExhaustive({
      TickedFrame: ({ deltaTimeMs }) => {
        ...
        return [nextModel, ambientSpawnCommands(advancedParticles.length)]
      },

      SpawnedParticle: spawn => {
        ...
        return [
          evo(model, {
            particles: particles => Array.append(particles, newParticle),
      ...
      PressedCanvas: ({ x, y }) => [
        model,
        burstCommands(x, y, computeBurstHueAnchor(model.elapsedSeconds)),
      ],

      MovedPointer: ({ x, y }) => [
        evo(model, {
          maybeMousePosition: () => Option.some<Point>({ x, y }),
        }),
        [],
      ],
      ...
      ClickedReset: () => [
        evo(model, {
          particles: () => [],
          maybeMousePosition: () => Option.none<Point>(),
        }),
        [],
      ],

      ChangedFlowStrength: ({ value }) => [
        evo(model, {
          flowStrength: () =>
            clamp(value, FLOW_STRENGTH_MIN, FLOW_STRENGTH_MAX),
        }),
        [],
      ],

      ChangedNoiseScale: ({ value }) => [
        evo(model, {
          noiseScaleMultiplier: () =>
            clamp(value, NOISE_SCALE_MIN_DIVISOR, NOISE_SCALE_MAX_DIVISOR),
        }),
        [],
      ],
    }),
