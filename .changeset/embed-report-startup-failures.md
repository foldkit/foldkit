...
`embed` now mirrors that reporting: a failed startup logs the Cause, interrupt-only exits from `dispose` stay quiet, and the public handle API is unchanged. `@foldkit/vite-plugin` force-includes `effect/Runtime` so the new import stays in the prebundled Effect blob in dev.
