...
The template is now taken out of the browser bundle before anything is written, and the browser output holds a page at `/` only when the build generated one through `prerender`. The server bundle keeps the template private: it is not a document, and a host that needs one renders it by calling the bundle's `fetch`. A generation loop of your own over an `ssr.build` output does the same, one `Request` per path, and never reads a template. Everything else about the build is unchanged: the handler renders into the same template, `prerender` writes the same pages to the same paths, and `foldkit.build.json` still records which paths became files. A host serves the files it finds and calls `fetch` for the rest, which is what the Node reference server did already.
