...
The template is now taken out of the browser bundle before anything is written, and the browser output holds a page at `/` only when the build generated one through `prerender`. Everything else about the build is unchanged: the handler renders into the same template, `prerender` writes the same pages to the same paths, and `foldkit.build.json` still records which paths became files. A host serves the files it finds and calls `fetch` for the rest, which is what the Node reference server did already.
