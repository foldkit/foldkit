...
`inputs` (optional second view argument) carries slot content built in the parent's boundary. Top-level function values in `inputs` are auto-wrapped to execute in the parent's boundary so user-provided handlers inside slots dispatch through the user's chain, not the embedded Submodel's. Functions nested below the top level are rejected at compile time by a mapped type on `SubmodelConfig.inputs`, with the same shape also enforced at view-build time as a defensive net for type-escape cases (`as any` casts, dynamic input construction).
