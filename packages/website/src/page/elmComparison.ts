  ...
  return h.div(
    ...
    [
      ...
      para(
        ...
        ') in Foldkit, in ',
        link(reactComparisonRouter(), 'React'),
        ', and in Elm. Same features, same styling, same algorithms: grid drawing with undo/redo stacks, three tools with mirror modes, flood fill, localStorage persistence, PNG export, keyboard shortcuts, and a time-travel history panel.',
      ),
      para(
        'This page is different from the React comparison. Foldkit and React disagree about architecture, so that page is an argument. Foldkit and Elm agree about architecture, so this page is a translation guide. Foldkit is the Elm Architecture, implemented in TypeScript on top of Effect: a single Model, Messages as facts, a pure update function, side effects as data the runtime executes. If you know Elm, you already know how a Foldkit app is shaped. What differs is the host language and what each side gets from it.',
      ),
      para(
        ...
        [
          ...
          [
            ...
            [
              ...
              ', CustomElement (no boundary)',
            ],
          ],
          [
            ['Nested state'],
            ['Nested TEA (by hand)'],
            [link(coreSubmodelRouter(), 'Submodel'), ' conventions + helpers'],
          ],
        ],
      ),
      para(
      ...
      para(
        'The Foldkit Message union has 30. Same naming convention, same facts, same role as the total input domain of the update function:',
      ),
      highlightedCodeBlock(
        h.div(
          [
      ...
      para(
        'Why 30 against 21? Nine of the difference is structural, not stylistic. Foldkit Commands report back: ',
        inlineCode('CompletedSaveCanvas'),
        ' and ',
        inlineCode('SucceededExportPng'),
        ' exist because every Command resolution returns to update as a Message. In Elm, a fire-and-forget port has no completion Msg unless you wire one through another port. The other eight are ',
        inlineCode('Got*Message'),
        ' variants delegating to ',
        link(uiOverviewRouter(), 'Foldkit UI'),
        ' Submodels (Dialog, RadioGroup, Switch, Listbox). The Elm version hand-rolls those components, so their events collapse into the app’s own Msg variants, plus one Msg Foldkit doesn’t need: ',
        inlineCode('ToggledThemePicker'),
        ', because the hand-rolled listbox tracks its own open state in the Model.',
      ),
      para(
      ...
      para(
        'Exhaustiveness deserves an honest note. Elm’s compiler enforces it at the language level: a missing case is a compile error, full stop. Foldkit gets the same failure mode through ',
        inlineCode('M.tagsExhaustive'),
        ', which makes a missing Message handler a type error. In practice both stop you at compile time. The difference is that Elm’s guarantee is unconditional, while Foldkit’s holds as long as you stay inside the conventions (which the framework, lint rules, and these docs push you toward).',
      ),
      ...
      para(
        ...
        ' is ',
        inlineCode('Just'),
        ', the confirm dialog exactly when ',
        inlineCode('pendingGridSize'),
        ' is. No booleans to desynchronize.',
      ),
      highlightedCodeBlock(
        h.div(
          [
      ...
      para(
        'Why declare the Model as a Schema instead of a plain type? Because a Schema is a type that also exists at runtime. It validates data at the boundaries (the localStorage round-trip below), derives encoders and decoders instead of making you write them, and gives DevTools a faithful, serializable picture of your whole application state. Elm’s type system is cleaner to read and arguably sounder. Foldkit’s buys you machinery.',
      ),
      ...
      para(
        'The cost is also real. The export feature is now two codebases: an encoder and port declaration in Elm, and an untyped JavaScript handler somewhere else that the compiler has never heard of. The failure path needs its own port. Rename a field in the encoder and the JavaScript silently receives the old shape of nothing. The app’s most interesting effect logic (paint the grid to a canvas, trigger a download) lives in the one file with no type checker, no tests, and no module system.',
      ),
      ...
      para(
        ...
        ': named values wrapping an Effect, with the canvas code written inline, typed, and tested alongside everything else.',
      ),
      highlightedCodeBlock(
        h.div(
          [
      ...
      infoCallout(
        ...
        'Elm’s purity at this boundary is enforced; Foldkit’s is conventional. The convention is concrete, though: side effects exist only inside Commands, every Command resolves to a Message (including failures, via ',
      ...
      para(
        'Elm requires a decoder for the way in and an encoder for the way out, written separately, kept in sync by discipline:',
      ),
      highlightedCodeBlock(
        h.div(
          [
      ...
      para(
        ...
        ' make the identical bet: if the arguments are reference-equal to last render, skip the view function. Elm’s record update and Foldkit’s ',
        inlineCode('evo'),
        ' both preserve unchanged references, so the bet pays off in both. The mechanical difference: Elm’s ',
        inlineCode('lazy'),
      ...
      para(
        'One layer down, the per-cell code is where the two frameworks look most alike, because both treat messages as values at the event boundary. No handler closures, nothing to stabilize, in either:',
      ),
      highlightedCodeBlock(
        h.div(
          [
      ...
      comparisonTable(
        ...
        [
          ...
          [
            ['Wiring pattern'],
            ['Nested TEA by hand'],
            ['Same pattern, standardized helpers'],
          ],
        ],
      ),
      ...
      para(
        'If you move from Elm to Foldkit, these are real losses. We’d rather you hear them from us.',
      ),
      para(
        ...
        inlineCode('Story'),
        ' tests, and review culture catch it; Elm’s compiler simply forbids it. Foldkit’s purity is a discipline with good tooling. Elm’s is a law of physics.',
      ),
      para(
      ...
      para(
        ...
        inlineCode('any'),
        ', no type assertions, no six ways to write a function, one formatter with no options, and packages that physically cannot perform side effects. TypeScript plus Effect is a far larger surface, and Foldkit’s conventions (this site’s best-practices section exists for a reason) are doing work the Elm language does by construction.',
      ),
      para(
      ...
      para(
        ...
        'A Foldkit program can mount inside an existing TypeScript app, share its types, and call its modules. Teams that could never justify “rewrite in Elm” can justify “this feature is a Foldkit program”.',
      ),
      para(
      ...
      para(
        'The bet Foldkit makes is that the Elm Architecture is the most durable idea in frontend, worth carrying into the ecosystem where most teams actually live, even at the cost of trading compiler-enforced guarantees for convention-enforced ones. You can audit that trade concretely: read ',
        link(
          'https://github.com/foldkit/foldkit/tree/main/comparisons/pixel-art-elm',
          'the Elm source',
        ),
