  ...
  return h.div(
    ...
    [
      ...
      para(
        ...
        ' inside its Effect. Both placements work, so the question is which one a given service should use. The deciding lens: is the service an app-wide singleton every Command should reuse, or a per-Command decision where different Commands want different implementations? Four criteria sharpen the call.',
      ),
      bullets(
        h.span(
          [],
      ...
      para(
        ...
        ' defaults to per-Command: ',
        inlineCode('Effect.provide(Http.layer)'),
        ' is one line, the Command stays self-contained, and no type annotations change anywhere else. ',
        inlineCode('Http.layer'),
        ' from ',
        inlineCode('foldkit/http'),
        ' is Effect’s Fetch-backed client with trace header propagation disabled by default. Effect tunes that default for servers, where handing trace context to your own downstream services is desirable; in the browser the same ',
        inlineCode('traceparent'),
        ' headers make otherwise CORS-simple requests trigger preflights against plain APIs and dev proxies. ',
        inlineCode('KeyValueStore'),
      ...
      highlightedCodeBlock(
        ...
        'Copy per-Command HTTP example to clipboard',
        copiedSnippets,
        'mb-8',
      ),
      para(
        'An HTTP client can still graduate. When an app grows many HTTP Commands, or shares a derived ',
        inlineCode('HttpApiClient'),
        ' across modules, provide ',
        inlineCode('Http.layer'),
        ' once via ',
        inlineCode('resources'),
        ' instead. The moment you find yourself writing a ',
        inlineCode('withClient'),
        ' helper to cut the repetition is the signal. Providing at the edge also helps tests: the Command’s Effect keeps ',
        inlineCode('HttpClient'),
        ' in its requirements, so an Effect-level test can provide a mock with ',
        inlineCode('Layer.succeed(HttpClient.HttpClient, mockClient)'),
        ' directly. A per-Command provide needs a separately exported raw Effect to test the same way.',
      ),
