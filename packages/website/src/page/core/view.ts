...
export const view = (copiedSnippets: CopiedSnippets): Html => {
  ...
  return h.div(
    ...
    [
      ...
      para(
        'A composite widget that owns the keyboard for a region but embeds interactive children inside it has a problem: keydown bubbles, so the host’s handler fires for keys typed in a descendant control and classifies them as its own input. ',
        inlineCode('OnKeyDownSelf'),
        ' and ',
        inlineCode('OnKeyDownSelfPreventDefault'),
        ' fix this by firing only when the keydown targets the host itself (',
        inlineCode('event.target === event.currentTarget'),
        '). For a contenteditable host the focused element is the host, so its own typing fires the handler while keys typed in an embedded input bubble through untouched. They are otherwise identical to ',
