...
export const view = (copiedSnippets: CopiedSnippets): Html =>
  h.div(
    ...
    [
      ...
      para(
        'CustomElement is the right primitive when the foreign element has a declarative API: typed JS properties going in, ',
        inlineCode('CustomEvent'),
        's coming out. Everything you push to the element is a property; everything you read back is an event. The element owns its rendering and its internal state, and Foldkit sees the same surface a vanilla JS or React consumer would. Most modern web component libraries (Shoelace, UI5, Spectrum, Lit, Stencil) fit this shape by design.',
      ),
      para(
        link(coreMountRouter(), 'Mount'),
        ' stays the right primitive when the foreign API is imperative: when you need to call ',
        inlineCode('new Map({ container }).flyTo(...)'),
        ', ',
        inlineCode('editor.setValue(text)'),
        ', ',
        inlineCode('dialog.showModal()'),
        ', or otherwise drive the element through method calls rather than property writes. ',
        inlineCode('OnMount'),
        ' hands you the live ',
        inlineCode('Element'),
        ' so you can instantiate the library, call methods on it, and pair a cleanup. The ',
