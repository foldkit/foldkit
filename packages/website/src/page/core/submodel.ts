  ...
  return h.div(
    ...
    [
      ...
      para(
        ...
        ' instance are helpers a parent calls without ever constructing the child’s Message, so the child can restructure its Messages later without touching any consumer. Each is a thin wrapper over the child’s ',
        inlineCode('update'),
      ...
      para(
        ...
        link(`${bestPracticesImmutabilityRouter()}#immutable-updates`, 'evo'),
        ' callback. The parent reflects URL-owned price bounds onto a Slider from the handler that processes the route change:',
      ),
      highlightedCodeBlock(
        h.div(
          [
      ...
      para(
        ...
        '), and they emit. The ',
        inlineCode('reflect*'),
        ' family is the uniform name for the silent inbound setter: the ',
        inlineCode('reflectMinDate'),
        ' / ',
        inlineCode('reflectMaxDate'),
        ' / ',
        ...
        ' / ',
        inlineCode('reflectDisabledDaysOfWeek'),
        ' constraint setters (Calendar, DatePicker), and ',
