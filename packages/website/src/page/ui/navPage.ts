    ...
    return h.div(
      ...
      [
        ...
        para(
          ...
          '. Use it for a bottom tab bar or a side nav.',
        ),
        infoCallout(
          'Switching content within one page?',
          'Reach for ',
          link(uiTabsRouter(), 'Tabs'),
          ' instead. Nav is for moving between pages; Tabs is for swapping panels inside a single page.',
        ),
        infoCallout(
          'See it in an app',
          'The ',
        ...
        demoContainer(...Nav.basicDemo(model.navDemoSection)),
        ...
        para(
          'Both render a row of selectable items, but the semantics differ. ',
          link(uiTabsRouter(), 'Tabs'),
          ' applies ',
          ...
          inlineCode('aria-current="page"'),
          ': it moves between separate routes, holds no state of its own, and leaves each link in the tab order. If each item has its own URL and the browser back button should move between items, use Nav.',
        ),
        heading(
