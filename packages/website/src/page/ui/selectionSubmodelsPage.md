...
:::Info{label="Where to declare it in a split app"}
In a single-file page, keep the factory next to its Model, update, and view. When a page is split across `update.ts` and `view/` files, put the factory in a `factories.ts` file at the page root and import that shared bundle from both. Declaring it in `model.ts` is also reasonable when the project colocates Model-bound definitions there. Pick one location and use it consistently.
:::
