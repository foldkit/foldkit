...
GotSliderMessage: ({ message }) => {
  const [nextSlider, commands] = Ui.Slider.update(model.painDemo, message)

  return [
    // Merge the next state into your Model:
    evo(model, { painDemo: () => nextSlider }),
...
>({
  sliderPointer: {
    modelToDependencies: model =>
      sliderSubs.documentPointer.modelToDependencies(model.painDemo),
    dependenciesToStream: (deps, readDeps) =>
      sliderSubs.documentPointer
        .dependenciesToStream(deps, readDeps)
        .pipe(Stream.map(message => GotSliderMessage({ message }))),
  },
  sliderEscape: {
    modelToDependencies: model =>
      sliderSubs.documentEscape.modelToDependencies(model.painDemo),
    dependenciesToStream: (deps, readDeps) =>
      sliderSubs.documentEscape
        .dependenciesToStream(deps, readDeps)
        .pipe(Stream.map(message => GotSliderMessage({ message }))),
  },
})
    ...
    div(
      ...
      [
        div(
          ...
          [
            label([...attributes.label, Class('font-medium')], ['Pain level']),
            span(
              [Class('tabular-nums text-gray-600')],
              [`${String(model.painDemo.value)} / 10`],
            ),
          ],
        ),
        div(
