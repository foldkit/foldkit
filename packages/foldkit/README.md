...
- **Submodels**: A pattern for composing nested modules. A child owns its own Model, Messages, update function, and view; the parent embeds it and wraps child Messages in a `Got*Message` envelope. `Html.mapMessage` remaps a sealed child view's Messages into the parent's, mirroring `Command.mapEffect` and `Mount.mapMessage` and letting internal view helpers stay non-generic. The pattern scales unchanged from a login form to a multi-page app.
