...
- **Slow Warnings**: Wire `slow` on `makeApplication` or `makeElement` to catch synchronous phases that exceed budgets you set: `update`, `view`, `patch`, and per-subscription `modelToDependencies`. The callback receives a tagged `SlowContext`, so one sink can route every slow event to logs, Sentry, or an in-app profiler.
