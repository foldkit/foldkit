...
- **Slow-Phase Monitoring**: Wire `slow` on `makeProgram` to catch the synchronous phases of the update cycle that drift past their budget: `update`, `view`, `patch`, and per-subscription `modelToDependencies`. Each phase has its own threshold and can be enabled or disabled independently. A top-level `onSlow` callback receives a tagged `SlowContext` union, so you can forward every slow event to one observability sink with a single function.
