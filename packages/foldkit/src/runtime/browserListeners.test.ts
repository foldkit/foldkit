...
describe('addLinkClickListener', () => {
  ...
  it('dispatches an Internal request for a same-document fragment link', () => {
    const link = makeLink(`${window.location.href}#faq`)
    const event = click(link)

    expect(event.defaultPrevented).toBe(true)
    expect(dispatched).toHaveLength(1)
    expect(dispatched.map(request => request._tag)).toEqual(['Internal'])
  })
})
...
describe('addLinkClickListener without onUrlRequest', () => {
  ...
  it('leaves a same-document fragment link to the browser', () => {
    const link = makeLink(`${window.location.href}#faq`)
    const event = click(link)
  ...
  it('leaves an empty fragment link to the browser', () => {
    const link = makeLink('#')
    const event = click(link)
  ...
  it('pushes a fragment link to another path', () => {
    const link = makeLink(`${window.location.origin}/other#faq`)
    const event = click(link)

    expect(event.defaultPrevented).toBe(true)
    expect(dispatched).toHaveLength(0)
    expect(window.location.pathname).toBe('/other')
    expect(announcedPaths).toEqual(['/other'])
  })

  it('pushes a fragment link with a different search', () => {
    const { origin, pathname } = window.location
    const link = makeLink(`${origin}${pathname}?tab=2#faq`)
    const event = click(link)

    expect(event.defaultPrevented).toBe(true)
    expect(dispatched).toHaveLength(0)
    expect(window.location.search).toBe('?tab=2')
    expect(announcedPaths).toEqual([pathname])
  })
})
