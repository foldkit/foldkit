...
describe('types', () => {
  ...
  it('getOrIdle collapses an Option of AsyncData to the same AsyncData type', () => {
    const maybeNotes: Option.Option<State> = Option.some(success)
    expectTypeOf(AsyncData.getOrIdle(maybeNotes)).toEqualTypeOf<State>()
  })
...
describe('getOrIdle', () => {
  it('returns the entry when the Option holds one', () => {
    expect(AsyncData.getOrIdle(Option.some(success))).toBe(success)
    expect(AsyncData.getOrIdle(Option.some(stale))).toBe(stale)
  })

  it('collapses a missing keyed-cache entry to Idle', () => {
    const cache = HashMap.empty<string, State>()
    expect(AsyncData.getOrIdle(HashMap.get(cache, 'notes:1'))).toEqual(
      AsyncData.Idle(),
    )
  })
})
