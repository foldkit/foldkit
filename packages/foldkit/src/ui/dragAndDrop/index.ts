...
>({
  documentPointer: {
    ...
    dependenciesToStream: ({ dragActivity, orientation }) => {
      ...
      // NOTE: prevents text selection on non-draggable elements (e.g. column
      // headers) while the pointer moves during a drag. Applied on stream
      // creation, removed on stream interruption when drag ends.
      const textSelectionLock = Stream.async<never>(_emit => {
      ...
      return Stream.when(
        Stream.merge(pointerEvents, textSelectionLock),
        () => dragActivity === 'Active',
      )
    },
  },
