...
/** The user pressed a pointer inside the slider (thumb or track). Starts a
 *  drag, and for track presses snaps the value to the cursor position. */
...
/** Creates an initial slider model from a config. The initial value defaults
 *  to `min` and is clamped into range. */
export const init = (config: InitConfig): Model => {
  const step = config.step ?? DEFAULT_STEP
  const rawValue = config.initialValue ?? config.min
  return {
    id: config.id,
    value: snapAndClamp(rawValue, config.min, config.max, step),
    min: config.min,
    max: config.max,
    step,
    dragState: Idle(),
...
const stepDecimals = (step: number): number => {
  ...
  const maybeDotIndex = text.indexOf('.')
  if (maybeDotIndex === -1) {
    return 0
  }
  return text.length - maybeDotIndex - 1
}
...
// PAGE_STEP_MULTIPLIER controls how far PageUp/PageDown jump compared to a
// single arrow-key step. Per the WAI-ARIA slider pattern, Page keys should
// move in a "larger" increment; 10 matches the common convention.
  ...
  M.value(message).pipe(
    ...
    M.tagsExhaustive({
      PressedPointer: ({ value }) => {
        // When PressedPointer fires on the thumb, it also bubbles to the
        // track — so both handlers dispatch. Preserve the existing
        // originValue when already Dragging so the second dispatch (and any
        // future stray PressedPointer during a drag) cannot overwrite it.
        const originValue = M.value(model.dragState).pipe(
          M.withReturnType<number>(),
          M.tag('Dragging', ({ originValue }) => originValue),
        ...
        return [
          evo(modelWithValue, {
            dragState: () => Dragging({ originValue }),
          }),
          commands,
          maybeOut,
        ]
...
>({
  documentPointer: {
    ...
    dependenciesToStream: ({ dragActivity }, readDependencies) => {
      const pointerEvents = Stream.merge(
        Stream.fromEventListener<PointerEvent>(document, 'pointermove').pipe(
          Stream.mapEffect(event =>
            Effect.sync(() => {
              const { id, min, max } = readDependencies()
              return Option.map(trackElement(id), element =>
                MovedDragPointer({
          ...
          Stream.filterMap(maybeMessage => maybeMessage),
        ),
...
export const view = <Message>(config: ViewConfig<Message>): Html => {
  ...
  const resolveThumbLabel = (): ReadonlyArray<Attribute<Message>> => {
    ...
    if (config.ariaLabelledBy !== undefined) {
  ...
  const thumbInteractionAttributes = isDisabled
    ? []
    : [OnPointerDown(pointerHandler), OnKeyDownPreventDefault(handleKeyDown)]
