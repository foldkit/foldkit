      ...
      Match.value(attr).pipe(
        Match.tagsExhaustive({
          ...
          OnMount: ({ action }) => {
            ...
            const notifyStarted = Option.match(maybeTracker, {
              onNone: () => Function.constVoid,
              onSome: tracker => () => tracker.started(action.name),
            })
            const notifyEnded = Option.match(maybeTracker, {
              onNone: () => Function.constVoid,
              onSome: tracker => () => tracker.ended(action.name),
            })
