...
describe('composeBoundary', () => {
  it('appends a child id under the root boundary', () => {
  ...
  it('throws when the child id contains the boundary separator', () => {
    // NOTE: without this check, an id like "row|inner" composed under
    ...
    expect(() => composeBoundary('list', 'row|inner')).toThrow(
      /h\.submodel id cannot contain the boundary separator/,
    )
  })

  it('throws when an id contains the separator even under the root boundary', () => {
