...
describe('no-hardcoded-route-strings', () => {
  it.each([
    ...
    'navigateInternal',
    'loadExternalUrl',
  ])('flags a hardcoded path literal in %s', functionName => {
  ...
  it.each(['pushUrl', 'replaceUrl', 'navigateInternal'])(
  ...
  it('flags an http URL in internal navigation', () => {
    const result = Testing.runRule(
      noHardcodedRouteStrings,
      'CallExpression',
      Testing.callExpr('navigateInternal', [
        Testing.strLiteral('http://example.com'),
      ]),
    )
  ...
  it.each(['Href', 'load', 'openUrl', 'loadExternalUrl'])(
    'allows an absolute URL in external-facing call %s',
    functionName => {
      const result = Testing.runRule(
        noHardcodedRouteStrings,
        'CallExpression',
