...
- **Vite plugin relay** (in `@foldkit/vite-plugin`): serves a WebSocket endpoint on the dev server itself, publishes its address to a per-user registry, and forwards traffic between browsers and MCP clients. The address carries a token the relay requires, so a dev server on `--host` does not expose the relay to the network. `devToolsMcpPort` opens a socket of its own on a fixed port instead, without a token.
