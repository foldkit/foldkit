...
/** Union of OutMessages the popover component can produce. Parents reacting
 *  to open/close transitions (for example, to reset related state or fire
 *  analytics) read this from the optional `outMessage` field of `update`'s
 *  return. */
...
export const update = (model: Model, message: Message) => {
  ...
  const closePopover = (
    ...
    return { model: closed, commands, outMessage: OutMessage.Closed() }
  }
  ...
  return Message.match<UpdateReturn>(message, {
    ...
    PressedPointerOnButton: ({ pointerType, button }) => {
      ...
      if (model.isOpen) {
        const closePopoverResult = closePopover(
          withPointerType,
          closeWithFocusCommands,
        )
        return {
          ...closePopoverResult,
          model: evo(closePopoverResult.model, {
            maybeLastButtonPointerType: () => Option.some(pointerType),
          }),
        }
      }
