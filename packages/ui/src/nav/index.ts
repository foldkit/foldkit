 ...
 *
 *  Use this for between-page navigation (a bottom tab bar, a side nav) where
 *  each section is a separate route with its own URL, deep-linking, and
 *  browser-history behavior. Reach for `Tabs` instead when switching content
 *  within a single page; `Tabs` carries the `tablist`/`tab`/`tabpanel`
 *  semantics, which are wrong for between-page navigation.
 *
  ...
  const itemInfos: ReadonlyArray<ItemInfo<Value>> = Array.map(
    ...
    (value, index) => {
      ...
      const linkAttributes = [
        ...
        ...(isCurrent
          ? [h.AriaCurrent(ariaCurrentPage), h.DataAttribute('current', '')]
          : []),
      ]
