...
## Gate commands (run ALL THREE; fix everything they surface)
...
"Typecheck clean and tests pass" is NOT sufficient. Generated code frequently has unused imports (`Invalid`, `NotValidated`, `Valid` imported as values when only used as string-literal tag keys in `M.tag(...)`) that only the linter catches. Skipping lint means shipping a file that ESLint would reject on the first `npm run lint` the user runs.
