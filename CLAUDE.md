...
- Branch identity is automatic. The `@foldkit/vite-plugin` build injects a call-site key on every conditional view arm that constructs its element directly (ternary arms, `if`/`else` returns, `Match` arms, `Option.match` / `Array.match` handlers, inline conditional inserts). Do not add manual keys to those arms; switching branches replaces the subtree on its own.
- Key delegated arms at the branch site. When a branching position's arms delegate to other view functions (route matches are the common case), the build cannot see the element construction, so wrap the result once with the discriminating key: `h.keyed('div')(model.route._tag, [], [routeContent])`. The `keyed-required-for-delegated-arms` lint flags the gap. Conditional inserts of delegated content between stable siblings need the same treatment.
- Key mapped list items by a stable model identifier, never by array position. Injected keys deliberately skip mapped rows; a row's identity has to come from the Model.
- Keys carry identity, never data. A key answers which branch or item occupies a position, not what it currently shows. Never derive a key from displayed data to force a refresh; content changes patch in place, and a data-derived key tears down live DOM state (focus, scroll, an open `details`) on every change.
